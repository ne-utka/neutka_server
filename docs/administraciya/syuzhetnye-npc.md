...
`dialog_range` задаёт максимальное расстояние разговора от текущей позиции NPC. По умолчанию это 6 блоков. После выхода за границу игрок получает короткое предупреждение и 0,5 секунды на возвращение: это защищает от случайного разрыва на самой границе и небольших сетевых рывков. Если игрок не вернулся, система останавливает текущий звук, убирает варианты ответа и завершает диалог. Исчезновение NPC или переход игрока в другой мир завершает разговор сразу. Команда `/story dialog play`, запущенная без привязанного NPC, этим ограничением не затрагивается.
