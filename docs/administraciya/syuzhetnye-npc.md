...
`start` задаёт первый узел. После последовательного воспроизведения `lines` система либо автоматически переходит по `next`, либо показывает кликабельные `choices`. Варианты появляются в мире справа от NPC и видны только участнику разговора: наведение выделяет строку золотой плашкой, ПКМ подтверждает выбор. В чат варианты не отправляются. Каждый вариант содержит видимый `text` и ID следующего узла в `next`. Значение `end` безопасно завершает разговор.
