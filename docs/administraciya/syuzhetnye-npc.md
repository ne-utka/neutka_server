...
`start` задаёт первый узел. После последовательного воспроизведения `lines` система либо автоматически переходит по `next`, либо показывает кликабельные `choices`. Каждый вариант содержит видимый `text` и ID следующего узла в `next`. Значение `end` безопасно завершает разговор.
