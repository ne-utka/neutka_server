...
#ifdef CAZ_POST_SHARPEN
    // CazToon's DEFAULT profile runs this after the outline. Besides making
    // the generated contour crisper, it strengthens nearby geometry, AO and
    // texture transitions that visually read as a wider toon edge.
    ivec2 imageSize = textureSize(colortex0, 0);
    ivec2 lo = ivec2(0);
    ivec2 hi = imageSize - ivec2(1);
    vec3 north = texelFetch(colortex0, clamp(pixel + ivec2( 0,  1), lo, hi), 0).rgb;
    vec3 south = texelFetch(colortex0, clamp(pixel + ivec2( 0, -1), lo, hi), 0).rgb;
    vec3 east  = texelFetch(colortex0, clamp(pixel + ivec2( 1,  0), lo, hi), 0).rgb;
    vec3 west  = texelFetch(colortex0, clamp(pixel + ivec2(-1,  0), lo, hi), 0).rgb;
    vec3 neighbourAverage = (north + south + east + west) * 0.25;
    scene.rgb += (texelFetch(colortex0, pixel, 0).rgb - neighbourAverage) * CAZ_POST_SHARPEN_STRENGTH;
#endif

    gl_FragColor = vec4(clamp(scene.rgb, vec3(0.0), vec3(1.0)), scene.a);
}
